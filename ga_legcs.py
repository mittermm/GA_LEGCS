...
# pop_size = 1000
# 
# print("load initial population")
# initial_population = get_initial_population(range(pop_size))
# 
# graph_list = []
# for edge_list in initial_population:
#     graph_list.append(get_graph_data(edge_list))
# 
# sims_list = get_sims()
# sims_list = sims_list[:pop_size]
# print("done\nstart genetic algorithm")
# 
# normalized_sims, n_min, n_max = normalize_list(sims_list)
# ga = GeneticAlgorithm3(pop_size, 0.3, initial_population, normalized_sims)
# 
# for gen in range(100):
#     print("start generation ", gen)
#     full_dataset = GraphDataset(graph_list, normalized_sims)
#     train_dataset, val_dataset = random_split(full_dataset, [0.8, 0.2])
#     model = get_model(GNN(), train_dataset, val_dataset)
# 
#     ga.evolve()
#     
#     preds = []
...
#         model.eval()
#         preds.append(model(get_graph_data(genome)).item())
#         
#     top_predictions = list(np.argsort(preds)[:int(0.01 * pop_size)])
#         
#     for index in top_predictions:
#         edge_list = ga.population[index]
#         
...
#         print("network gen" + str(gen) + " genome" + str(index) + ": "
...
#         
#         graph_list.append(get_graph_data(edge_list))
#         sims_list.append(time)
# 
#         print("pred ", preds[index], " to sim ", normalize(time, n_min, n_max))
#         
#         preds[index] = normalize(time, n_min, n_max)
#         
#     ga = GeneticAlgorithm3(pop_size, 0.3, ga.population, preds)    
#     normalized_sims, n_min, n_max = normalize_list(sims_list)
# 
